...
function matches(ticket: TicketFrontmatter, filters: MetricsFilters): boolean {
  ...
  if (filters.workflow_id && ticket.workflow?.id !== filters.workflow_id) return false;
  if (filters.workflow_revision && ticket.workflow?.revision !== filters.workflow_revision) return false;
...
export async function buildMetrics(store: TicketStore, workflows: WorkflowLibrary, filters: MetricsFilters) {
  ...
  for (const ticket of tickets) for (const run of ticket.workflow?.node_runs ?? []) {
    ...
    const metric = nodeMetrics.get(key) ?? {
      ...
      runs: 0, completed: 0, running: 0, interrupted: 0, bypassed: 0, lease_lost: 0,
      success: 0, failure: 0, neutral: 0, unclassified: 0, wall: [], active_ms: 0,
      quota_paused_ms: 0, human_wait_ms: 0, tokens: 0, token_runs: 0, cost: 0, cost_runs: 0,
      branches: new Map<string, MutableBranch>(),
    };
    ...
    metric.active_ms += run.timing.active_ms; metric.quota_paused_ms += run.timing.quota_paused_ms; metric.human_wait_ms += run.timing.human_wait_ms;
    ...
    if (run.outcome && run.outcome !== "bypassed" && run.outcome !== "lease_lost" && run.status !== "interrupted") metric[metricClass] += 1;
    if (run.outcome) {
    ...
    if (run.node_type === "agent" && run.provider !== null) {
      ...
      const alias = resolved?.alias ?? `legacy-${run.provider ?? "unknown"}`;
      ...
      if (metricClass === "success") profile.success += 1; if (metricClass === "failure") profile.failure += 1;
  ...
  return {
    ...
    available: {
      ...
      workflows: [...new Map(all.flatMap((ticket) => ticket.workflow ? [[`${ticket.workflow.id}@${ticket.workflow.revision}`, { id: ticket.workflow.id, revision: ticket.workflow.revision }]] : [])).values()],
    ...
    workflows: [...nodeMetrics.values()].reduce<Array<{ workflow_id: string; workflow_revision: string; ticket_ids: Set<string>; nodes: MutableNode[] }>>((groups, node) => {
      ...
      nodes: group.nodes.map((node) => {
        ...
        return {
          ...
          interrupted: node.interrupted, bypassed: node.bypassed, lease_lost: node.lease_lost,
          ...
          human_wait_ms: node.human_wait_ms, total_tokens: node.tokens, known_cost_usd: node.cost,
